  ...
  */
  try {
    ...
    if (!token) {
      ...
      const jwtVerify = jwt.verify(token, JWT_SECRET, (err, decodeToken) => {
        if (err) {
          next({
            status: 401,
            message: "Token gecersizdir",
...
const usernameVarmi = async (req, res, next) => {
  ...
  */
  try {
    let usernameCheck = await userModel.goreBul({
    ...
    if (!usernameCheck || usernameCheck.length === 0) {
      ...
      let user = usernameCheck.filter((x) =>
        bcyrpt.compareSync(req.body.password, x.password)
      );
      if (user.length == 0) {
        next({
          status: 401,
